...
function [Markers,Hwb] = DTRT_mo_gui_v3(signals_proc,spikes,ParamIn,Info_Correct);
...
%             end
            ...
            if do_RT_W||do_RT_A
                ...
                if ~isnan(tiso(j,i)+tr); % mo 17/06/2015
                    ...
                    A = detrend(X(xx,j,i));
                    A = A-nanmean( A(1:min(length(A),round(50/1000*ParamIn.frequency))));
            ...
            if do_Tend
                ...
                if iiTwpos(i) && ~isnan(tMmin(j,i))
                    ...
                    % 26/06/2019
                    Tm = [1:size(Xd,1)].' > x0+0.050*ParamIn.frequency & [1:size(Xd,1)].' < x0+0.150*ParamIn.frequency & abs(Xd([1:size(Xd,1)].',j,i))<max(abs(Xd([1:size(Xd,1)].',j,i)))*0.05;
                    V0 = nanmedian(X(Tm,j,i));
