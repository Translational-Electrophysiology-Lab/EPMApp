function output_txt = myfunctioncursor_markers_EnsiteClassic(~,event_obj)
...
if ax==handles.tag_mesh;
    ...
    %     [~,is] = sort(sum(abs(handles.matfile.geo.Nodes_ventricles  - repmat(pos,[size(handles.matfile.geo.Nodes_ventricles ,1),1])),2),'ascend');
    %     im = is(1);
    
    [~,im] = min(sum(abs(handles.matfile.geo.Nodes_ventricles  - repmat(pos,[size(handles.matfile.geo.Nodes_ventricles ,1),1])),2));
    ...
    %     aa = plot3(handles.tag_mesh,handles.matfile.geo.Nodes_ventricles(im,1),handles.matfile.geo.Nodes_ventricles(im,2),handles.matfile.geo.Nodes_ventricles(im,3),'xk','markersize',30);
    %     set(aa,'tag','datatip')
    % aa = plot3(handles.matfile.geo.Nodes_ventricles(im,1),handles.matfile.geo.Nodes_ventricles(im,2),handles.matfile.geo.Nodes_ventricles(im,3),'xk','markersize',30);
    %     set(aa,'tag','datatip')
