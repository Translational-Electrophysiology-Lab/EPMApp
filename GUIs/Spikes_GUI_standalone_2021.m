function varargout = Spikes_GUI_standalone_2021(varargin)

%% Spikes_GUI_standalone_2021(signals(:,iis(1:20)),ParamSig.frequency,spikes,ParamSig.Label(iis(1:20)),do_wait);

% SPIKES_GUI_STANDALONE_2021 MATLAB code for Spikes_GUI_standalone_2021.fig
%      SPIKES_GUI_STANDALONE_2021, by itself, creates a new SPIKES_GUI_STANDALONE_2021 or raises the existing
%      singleton*.
%
%      H = SPIKES_GUI_STANDALONE_2021 returns the handle to a new SPIKES_GUI_STANDALONE_2021 or the handle to
%      the existing singleton*.
%
%      SPIKES_GUI_STANDALONE_2021('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPIKES_GUI_STANDALONE_2021.M with the given input arguments.
%
%      SPIKES_GUI_STANDALONE_2021('Property','Value',...) creates a new SPIKES_GUI_STANDALONE_2021 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Spikes_GUI_standalone_2021_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Spikes_GUI_standalone_2021_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Spikes_GUI_standalone_2021

% Last Modified by GUIDE v2.5 16-Mar-2021 15:22:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Spikes_GUI_standalone_2021_OpeningFcn, ...
    'gui_OutputFcn',  @Spikes_GUI_standalone_2021_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Spikes_GUI_standalone_2021 is made visible.
function Spikes_GUI_standalone_2021_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Spikes_GUI_standalone_2021 (see VARARGIN)

% Choose default command line output for Spikes_GUI_standalone_2021
...
% UIWAIT makes Spikes_GUI_standalone_2021 wait for user response (see UIRESUME)
if do_wait
    uiwait(handles.figure1);
end

% --- Outputs from this function are returned to the command line.
function varargout = Spikes_GUI_standalone_2021_OutputFcn(hObject, eventdata, handles)
...
if get(handles.tag_do_QRS,'value')==0
    ...
    %% Sinus Rhythm
        ...
        [spikes,fwhm] = spikes_detection_SR_CinC(signals(:,1),handles.ParamSig.frequency);
        if ~isempty(spikes)
        %% plot
        tms = [1:size(signals,1)]/handles.ParamSig.frequency*1000;
        hold(handles.tag_ax_sig,'off')
        plot(handles.tag_ax_sig,tms,signals(:,1)),
        hold(handles.tag_ax_sig,'on')
...
% --------------------------------------------------------------------
function tag_zoom_out_ClickedCallback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function tag_zoom_in_ClickedCallback(hObject, eventdata, handles)
