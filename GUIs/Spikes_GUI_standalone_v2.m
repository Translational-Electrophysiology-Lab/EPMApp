function varargout = Spikes_GUI_standalone_v2(varargin)

%% Spikes_GUI_standalone(signals(:,iis(1:20)),ParamSig.frequency,spikes,ParamSig.Label(iis(1:20)),do_wait);

% SPIKES_GUI_STANDALONE_V2 MATLAB code for Spikes_GUI_standalone_v2.fig
%      SPIKES_GUI_STANDALONE_V2, by itself, creates a new SPIKES_GUI_STANDALONE_V2 or raises the existing
%      singleton*.
%
%      H = SPIKES_GUI_STANDALONE_V2 returns the handle to a new SPIKES_GUI_STANDALONE_V2 or the handle to
%      the existing singleton*.
%
%      SPIKES_GUI_STANDALONE_V2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPIKES_GUI_STANDALONE_V2.M with the given input arguments.
%
%      SPIKES_GUI_STANDALONE_V2('Property','Value',...) creates a new SPIKES_GUI_STANDALONE_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Spikes_GUI_standalone_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Spikes_GUI_standalone_v2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Spikes_GUI_standalone_v2

% Last Modified by GUIDE v2.5 20-Apr-2017 12:54:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Spikes_GUI_standalone_v2_OpeningFcn, ...
    'gui_OutputFcn',  @Spikes_GUI_standalone_v2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Spikes_GUI_standalone_v2 is made visible.
function Spikes_GUI_standalone_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Spikes_GUI_standalone_v2 (see VARARGIN)

% Choose default command line output for Spikes_GUI_standalone_v2
...
% quality index
[a,b] = butter(3,[0.1 30]/(handles.ParamSig.frequency/2));
...
% UIWAIT makes Spikes_GUI_standalone_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Spikes_GUI_standalone_v2_OutputFcn(hObject, eventdata, handles)
...
%     spikes = data.spikes;
%     save(data.outputname,'spikes','-append')
%     %
%     data.LOG = [data.LOG,['- saved : spikes']];
%     set(data.tag_Log,'string',data.LOG)
%     %
% end
...
elseif get(handles.tag_do_QRS,'value')
    ...
    [a,b] = butter(3,[0.1 30]/(handles.ParamSig.frequency/2));
    xflb = filtfilt(a,b,x);clear a b
    xf = medfilt1(diff(xflb).^2,round(50/1000*handles.ParamSig.frequency));
%     [a,b] = butter(3,30/(handles.ParamSig.frequency/2));
%     xf2 = filtfilt(a,b,xf);clear a b xf
