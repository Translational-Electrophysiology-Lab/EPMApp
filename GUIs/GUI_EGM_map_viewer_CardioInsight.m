function varargout = GUI_EGM_map_viewer_CardioInsight(varargin)
% GUI_EGM_MAP_VIEWER_CARDIOINSIGHT MATLAB code for GUI_EGM_map_viewer_CardioInsight.fig
%      GUI_EGM_MAP_VIEWER_CARDIOINSIGHT, by itself, creates a new GUI_EGM_MAP_VIEWER_CARDIOINSIGHT or raises the existing
%      singleton*.
%
%      H = GUI_EGM_MAP_VIEWER_CARDIOINSIGHT returns the handle to a new GUI_EGM_MAP_VIEWER_CARDIOINSIGHT or the handle to
%      the existing singleton*.
%
%      GUI_EGM_MAP_VIEWER_CARDIOINSIGHT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_EGM_MAP_VIEWER_CARDIOINSIGHT.M with the given input arguments.
%
%      GUI_EGM_MAP_VIEWER_CARDIOINSIGHT('Property','Value',...) creates a new GUI_EGM_MAP_VIEWER_CARDIOINSIGHT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_EGM_map_viewer_CardioInsight_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_EGM_map_viewer_CardioInsight_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_EGM_map_viewer_CardioInsight

% Last Modified by GUIDE v2.5 13-Jun-2017 16:14:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @GUI_EGM_map_viewer_CardioInsight_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_EGM_map_viewer_CardioInsight_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_EGM_map_viewer_CardioInsight is made visible.
function GUI_EGM_map_viewer_CardioInsight_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_EGM_map_viewer_CardioInsight (see VARARGIN)

% Choose default command line output for GUI_EGM_map_viewer_CardioInsight
...
% UIWAIT makes GUI_EGM_map_viewer_CardioInsight wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_EGM_map_viewer_CardioInsight_OutputFcn(hObject, eventdata, handles)
...
if ~isempty(hp)
    ...
    if isstruct(MV_lad)
        hold(handles.tag_mesh,'on')
        meshObject_LAD = patch(MV_lad,'parent',handles.tag_mesh);
        set(meshObject_LAD,'edgecolor','none','facecolor',[1 1 1]*0.2);
    end
    % Valves
    if isstruct(MV_valves)
        hold(handles.tag_mesh,'on')
        meshObject_valves = patch(MV_valves,'parent',handles.tag_mesh);
        set(meshObject_valves,'edgecolor','none','facecolor',[1 1 1]*0.2);
    end
...
if get(handles.tag_modify_type,'value')==1
    ...
    for j = 1:size(ii,2)
        ...
        if ~isempty(tt)
            ...
            % AT
            %             [~,dtp] = min(abs(t-dt));
            dtp = round(handles.matfile.Markers.dt(ib,ic(j))/1000*handles.matfile.ParamSig.frequency)+1;
