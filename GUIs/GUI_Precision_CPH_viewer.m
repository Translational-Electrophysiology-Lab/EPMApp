function varargout = GUI_Precision_CPH_viewer(varargin)
% GUI_PRECISION_CPH_VIEWER MATLAB code for GUI_Precision_CPH_viewer.fig
%      GUI_PRECISION_CPH_VIEWER, by itself, creates a new GUI_PRECISION_CPH_VIEWER or raises the existing
%      singleton*.
%
%      H = GUI_PRECISION_CPH_VIEWER returns the handle to a new GUI_PRECISION_CPH_VIEWER or the handle to
%      the existing singleton*.
%
%      GUI_PRECISION_CPH_VIEWER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_PRECISION_CPH_VIEWER.M with the given input arguments.
%
%      GUI_PRECISION_CPH_VIEWER('Propguideerty','Value',...) creates a new GUI_PRECISION_CPH_VIEWER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Precision_CPH_viewer_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Precision_CPH_viewer_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Precision_CPH_viewer

% Last Modified by GUIDE v2.5 17-Sep-2021 14:08:10

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @GUI_Precision_CPH_viewer_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_Precision_CPH_viewer_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_Precision_CPH_viewer is made visible.
function GUI_Precision_CPH_viewer_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Precision_CPH_viewer (see VARARGIN)

% Choose default command line output for GUI_Precision_CPH_viewer
...
if isfield(h,'signals_bipol')
    handles.matfile.signals_bipol = h.signals_bipol;
end
if isfield(h,'Markers_Ensite')
    handles.matfile.Markers_Ensite = h.Markers_Ensite;
end
if isfield(h,'spikes_all')
    handles.matfile.spikes_all = h.spikes_all;
end
...
% UIWAIT makes GUI_Precision_CPH_viewer wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Precision_CPH_viewer_OutputFcn(hObject, eventdata, handles)
...
if length(pos)>1
    im = cellfun(@(x) find(sum([xyz - x]==0,2)),pos,'un',0);
    im = cellfun(@(x) x(1),im);
...
if get(handles.tag_hold_on,'value')==1;
    ...
    if isnumeric(i_chan)
        ...
        xx_lim = [handles.matfile.spikes(ib)-300 handles.matfile.spikes(ib)+700]-handles.matfile.spikes(1);
    end
    yyaxis(handles.tag_egm,'right')
    cla(handles.tag_egm)
    yyaxis(handles.tag_egm,'left')
    cla(handles.tag_egm)
...
if handles.radiobutton_uni.Value==1
    aueg = plot(handles.tag_egm,t,S);
    set(aueg,'marker','none','linestyle','-','linewidth',1)
    for i = 1:length(aueg)
        set(aueg(i),'color',llc(i,:));
    end
    ...
    for j = 1:length(im)
        ...
        plot(handles.tag_egm,t(rtp)',S(rtp,j),'marker','x','markerfacecolor',[1 1 1]*.8,'markeredgecolor','k','linestyle','none','markersize',10,'linewidth',1)
    end
    ...
    if isfield(handles.matfile,'spikes_all')
        plot(handles.tag_egm,[1;1]*handles.matfile.spikes_all(im)-handles.matfile.spikes(1),get(handles.tag_egm,'ylim'),':','color',[.2 .5 .9]);
    end
    legend([aueg],[strleg(:)])
...
if handles.radiobutton_bipol.Value==1
    
    yyaxis(handles.tag_egm,'right');
    abi = plot(handles.tag_egm,t,handles.matfile.signals_bipol(:,im),'b');
    set(handles.tag_egm,'ycolor','b')
    set(abi,'marker','none')
    abi_at = gobjects(1,length(im));
    for i = 1:length(im)
        if ~isnan(handles.matfile.Markers_Ensite.LATsp(im(i)))
            abi_at(i) = plot(handles.tag_egm,[1;1]*t(handles.matfile.Markers_Ensite.LATsp(im(i))),[-1;1]*range(handles.matfile.signals_bipol(:,im(i)))/4,'m','linewidth',1);
        end
    end
    set(abi,'marker','none','linestyle','-','linewidth',0.5)
    set(abi_at(isgraphics(abi_at)),'marker','none','linestyle','-','linewidth',0.5)
    for i = 1:length(abi)w
        set(abi(i),'color',llc(i,:));
    end
    
    yl = [min(min(handles.matfile.signals_bipol(:,im))) max(max(handles.matfile.signals_bipol(:,im)))]+[-1 1] * .25 *min(range(handles.matfile.signals_bipol(:,im)));
    ylim_bi = yl;
    if ylim_bi(1) > - 0.5
        ylim_bi(1) = -.5;
    end
...
set(handles.tag_egm,'xgrid','on','ygrid','on');
...
switch str{istr}
    ...
    case 'LAT (Ensite)'
        values = handles.matfile.Markers_Ensite.LAT;
        colorLabel = 'LAT Ensite (ms)';
        set(handles.tag_slider,'value',0,'SliderStep',[0.01  0.1])
        handles.tag_modify_type.Value = 1;
        
    case 'Amp bipolar (Ensite)'
        values = handles.matfile.Markers_Ensite.Amp_p2p_bipol;
        colorLabel = 'Bipol Amp Ensite (mV)';
        set(handles.tag_slider,'value',0,'SliderStep',[0.01  0.1])
        handles.tag_modify_type.Value = 1;
...
dcm_obj = datacursormode(handles.figure1);
dcm_obj.Enable = 'on';
dcm_obj.UpdateFcn = @myfunctioncursor_markers_CARTO;
...
% Find beat
dd = pos(1) - (handles.matfile.spikes-handles.matfile.spikes(1));
dd(dd<0)=inf;
[~,ib] = min(dd);
...
llstr = ll.String;
llstr(~strncmp(llstr,'node',4))=[];
ic_all = nan(size(llstr));
for i=1:length(llstr)
    ic_all(i) = str2double(llstr{i}(6:end));
end
...
elseif isequal(get(info_struct.Target.Parent,'type'),'axes')
    ...
    pos = info_struct.Position;
    dt = pos(1) - (handles.matfile.spikes-handles.matfile.spikes(1));
    dt(dt<0)=inf;
    [~,ib] = min(dt);
    clear dt
    ...
    if ~isempty(ic)&~handles.tag_modify_all_chan.Value
    ...
    elseif ~isempty(ic)&&handles.tag_modify_all_chan.Value
    ...
    cc = findobj(handles.tag_mesh,'marker','o');
    xyz0 = [[cc(:).XData]' [cc(:).YData]' [cc(:).ZData]'];
    handles.matfile.geo.xyz(ic,:);
    ic0 = nan(size(ic));
    for j = 1:length(ic)
    ic0(j) = find(sum([handles.matfile.geo.xyz(ic(j),:)-xyz0],2)==0);
    end
    delete(cc(ic0));
    clear ic;
...
p_type = handles.tag_menu.String(handles.tag_menu.Value);
ii = sum(~cellfun(@isempty,strfind({'RVI','Disp'},p_type)));
if ii>0
    tag_RVI_Callback(hObject,[], handles);
end
...
% --------------------------------------------------------------------
function tag_import_point_xyz_Callback(hObject, eventdata, handles)
% hObject    handle to tag_import_point_xyz (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
xyz = nan(size(x));
for i = 1:length(x);
    xyz(i) = str2double(x{i});
end
hold(handles.tag_mesh,'on');
plot3(handles.tag_mesh,xyz(1),xyz(2),xyz(3),'x','markersize',20,'linewidth',2,'color','k');


% --- Executes on button press in radiobutton_uni.
function radiobutton_uni_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_uni (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_uni


% --- Executes on button press in radiobutton_bipol.
function radiobutton_bipol_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_bipol (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_bipol
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if isfield(handles.matfile,'w_whiskers')
    w = handles.matfile.w_whiskers;
else
    w = 1.5;
end
switch handles.tag_menu.String{handles.tag_menu.Value}
    case 'Activ. T'
        x = handles.matfile.Markers.dt-handles.matfile.spikes;
    case 'Repol. T'
        x = handles.matfile.Markers.rt_Wyatt-handles.matfile.spikes;
    case 'ARI'
        x = handles.matfile.Markers.rt_Wyatt-handles.matfile.Markers.dt;
end
xo = zeros(size(x));
...
po = findobj(handles.figure1,'marker','o');
delete(po)

xyz = handles.matfile.geo.xyz;
plot3(handles.tag_mesh,xyz(xo==1,1),xyz(xo==1,2),xyz(xo==1,3),'marker','o','markerfacecolor',cmap2(3,:),'linestyle','none','markeredgecolor','k');
plot3(handles.tag_mesh,xyz(xo==-1,1),xyz(xo==-1,2),xyz(xo==-1,3),'marker','o','markerfacecolor',cmap2(1,:),'linestyle','none','markeredgecolor','k');
plot3(handles.tag_mesh,xyz(xo==0,1),xyz(xo==0,2),xyz(xo==0,3),'marker','o','markerfacecolor',cmap2(2,:),'linestyle','none','markeredgecolor','k');

set(handles.tag_mesh,'clim',[-1 1])
cbar = findobj(handles.figure1,'tag','Colorbar');
cbar.Limits = [-1 1];
colormap(cmap2)
cbar.Ticks = [-1 0 1];
...
ccU = findobj(handles.tag_mesh,'type','line','MarkerFaceColor',[1 0 1]);
ccL = findobj(handles.tag_mesh,'type','line','MarkerFaceColor',[1 0 0]);

answer = questdlg('Do you want plot ...', ...
...
switch answer
    case 'Extremely Large values'
        if ~isempty(ccU)
            for i=1:length(ccU.XData)
                datatip(ccU,'dataindex',i);
                tag_plot_egm_Callback(hObject,[],handles);
            end
        end
    case 'Extremely Small values'
        if ~isempty(ccL)
            for i=1:length(ccL.XData)
                datatip(ccL,'dataindex',i);
                tag_plot_egm_Callback(hObject,[],handles);
            end
        end
        clear i
...
% --------------------------------------------------------------------
function tab_outlier_whisker_Callback(hObject, eventdata, handles)
% hObject    handle to tab_outlier_whisker (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
prompt = {'Enter "w" for whiskers length'};
dlgtitle = 'w = ';
definput = {'1.5'};
answer = inputdlg(prompt,dlgtitle,1,definput);
handles.matfile.w_whiskers = str2double(answer{1});
guidata(hObject, handles);

tag_outliers_Callback(hObject,[],handles);