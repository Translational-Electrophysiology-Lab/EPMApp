function varargout = GUI_EGM_map_viewer_Ensite(varargin)
% GUI_EGM_MAP_VIEWER_ENSITE MATLAB code for GUI_EGM_map_viewer_Ensite.fig
%      GUI_EGM_MAP_VIEWER_ENSITE, by itself, creates a new GUI_EGM_MAP_VIEWER_ENSITE or raises the existing
%      singleton*.
%
%      H = GUI_EGM_MAP_VIEWER_ENSITE returns the handle to a new GUI_EGM_MAP_VIEWER_ENSITE or the handle to
%      the existing singleton*.
%
%      GUI_EGM_MAP_VIEWER_ENSITE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_EGM_MAP_VIEWER_ENSITE.M with the given input arguments.
%
%      GUI_EGM_MAP_VIEWER_ENSITE('Property','Value',...) creates a new GUI_EGM_MAP_VIEWER_ENSITE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_EGM_map_viewer_Ensite_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_EGM_map_viewer_Ensite_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_EGM_map_viewer_Ensite

% Last Modified by GUIDE v2.5 01-Sep-2016 10:27:38

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @GUI_EGM_map_viewer_Ensite_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_EGM_map_viewer_Ensite_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_EGM_map_viewer_Ensite is made visible.
function GUI_EGM_map_viewer_Ensite_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_EGM_map_viewer_Ensite (see VARARGIN)

% Choose default command line output for GUI_EGM_map_viewer_Ensite
...
%
handles.matfile.geo.Nodes_ventricles = handles.matfile.ParamSig.Virtuals_geo.xyz;
handles.hObject_main = varargin{1};

DT = delaunayTriangulation( handles.matfile.ParamSig.Virtuals_geo.xyz);
[mesh_hull,v] = convexHull(DT); % mesh_original defines the faces used to do the mesh
handles.matfile.geo.Mesh_ventricles = mesh_hull;
...
% UIWAIT makes GUI_EGM_map_viewer_Ensite wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_EGM_map_viewer_Ensite_OutputFcn(hObject, eventdata, handles)
...
elseif get(handles.tag_view_mesh_interp,'value')
    %% Mesh 1
    MV = handles.matfile.ParamSig.Geometry.MESH;
    F = scatteredInterpolant(handles.matfile.geo.Nodes_ventricles(:,1),handles.matfile.geo.Nodes_ventricles(:,2),handles.matfile.geo.Nodes_ventricles(:,3),values(:));
    MV.FaceVertexCData  = F([MV.vertices(:,1),MV.vertices(:,2),MV.vertices(:,3)]);
else
    %% Mesh 2
    if size(handles.matfile.geo.Nodes_ventricles,1)==256
        N = 256-17;
        clear connectivity
        for i = 1:N
    ...
    elseif size(handles.matfile.geo.Nodes_ventricles,1)==64
        N = 64-9;
    ...
    elseif size(handles.matfile.geo.Nodes_ventricles,1)==2048
        N = 2048-65;
        clear connectivity
        for i = 1:N
...
if ~isempty(hp)
    ...
    axis(handles.tag_mesh,'off','equal')
...
if get(handles.tag_modify_type,'value')==1
    if ~isfield(handles.matfile,'signals_proc_Ensite_AT')
        ...
        x = handles.matfile.signals_proc_Ensite_AT(:,ic);
    end
    X = x(Ws,:);
    clear x
    ...
    if ~isfield(handles.matfile,'signals_proc_Ensite_RT')
        ...
        x = handles.matfile.signals_proc_Ensite_RT(:,ic);
    end
    X = x(Ws,:);
    clear x
    Xd = diff(X);Xd2 = diff(Xd);Xd3 = diff(Xd2);
...
elseif isequal(get(info_struct.Target.Parent,'type'),'axes')
    ...
    handles.matfile.Markers.rt_Wyatt(ib,ii(find(jj))) = nan;
    %
...
if handles.tag_view_mesh_interp.Value
    F = scatteredInterpolant(handles.matfile.geo.Nodes_ventricles(:,1),handles.matfile.geo.Nodes_ventricles(:,2),handles.matfile.geo.Nodes_ventricles(:,3),values(:));
    values_int  = F([po.Vertices(:,1),po.Vertices(:,2),po.Vertices(:,3)]);
else
    values_int  = values;
end
