function varargout = GUI_BSPM_map_viewer(varargin)
% GUI_BSPM_MAP_VIEWER MATLAB code for GUI_BSPM_map_viewer.fig
%      GUI_BSPM_MAP_VIEWER, by itself, creates a new GUI_BSPM_MAP_VIEWER or raises the existing
%      singleton*.
%
%      H = GUI_BSPM_MAP_VIEWER returns the handle to a new GUI_BSPM_MAP_VIEWER or the handle to
%      the existing singleton*.
%
%      GUI_BSPM_MAP_VIEWER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_BSPM_MAP_VIEWER.M with the given input arguments.
%
%      GUI_BSPM_MAP_VIEWER('Property','Value',...) creates a new GUI_BSPM_MAP_VIEWER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_BSPM_map_viewer_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_BSPM_map_viewer_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_BSPM_map_viewer

% Last Modified by GUIDE v2.5 21-Jul-2017 08:28:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @GUI_BSPM_map_viewer_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_BSPM_map_viewer_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_BSPM_map_viewer is made visible.
function GUI_BSPM_map_viewer_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_BSPM_map_viewer (see VARARGIN)

% Choose default command line output for GUI_BSPM_map_viewer
...
pc = computer;
if isequal(pc(1:2),'PC')
    load .\GUIs\BSM_MESH.mat
else
    load ./GUIs/BSM_MESH.mat
end

handles.matfile.geo.Mesh = MESH2;
handles.matfile.geo.xyz(:,1) = [handles.matfile.geo.Mesh.vertices_front(:,1);handles.matfile.geo.Mesh.vertices_back(:,1)];
handles.matfile.geo.xyz(:,2) = [handles.matfile.geo.Mesh.vertices_front(:,2);handles.matfile.geo.Mesh.vertices_back(:,2)];
...
% UIWAIT makes GUI_BSPM_map_viewer wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_BSPM_map_viewer_OutputFcn(hObject, eventdata, handles)
...
if get(handles.tag_hold_on,'value')==1
    ...
    xx_lim = [handles.matfile.spikes(ib)-500 handles.matfile.spikes(ib)+1500]-handles.matfile.spikes(1);
    cla(handles.tag_egm)
...
hold(handles.tag_mesh,'on')
for j = 1:length(im)
    aa(j) = plot(handles.tag_mesh,xyz(im(j),1),xyz(im(j),2),'x');
...
Clim = [prctile(values,2) prctile(values,99)];
if sum(isnan(Clim))>0
    warning('All nan')
    return
end
...
dcm_obj = datacursormode(handles.figure1);
set(dcm_obj,'UpdateFcn',@myfunctioncursor_markers_BSPM,'SnapToDataVertex','on')
cla(handles.figure1)
axes(handles.tag_mesh);

% split in front and back
valuesF = values(1:128);valuesF = valuesF(:);
valuesB = values(129:256);valuesB = valuesB(:);
% front
MV{1}.faces = handles.matfile.geo.Mesh.faces_front;
MV{1}.vertices =  handles.matfile.geo.Mesh.vertices_front;
% back
MV{2}.faces = handles.matfile.geo.Mesh.faces_back;
MV{2}.vertices =  handles.matfile.geo.Mesh.vertices_back;

if get(handles.tag_view_mesh_interp,'value')
    %% Meshes interp
    xyzF = handles.matfile.geo.Mesh.vertices_front;
    xyzB = handles.matfile.geo.Mesh.vertices_back;
    Ff = scatteredInterpolant(xyzF(~isnan(valuesF),1),xyzF(~isnan(valuesF),2),valuesF(~isnan(valuesF)));
    Fb = scatteredInterpolant(xyzB(~isnan(valuesB),1),xyzB(~isnan(valuesB),2),valuesB(~isnan(valuesB)));
    
    % front
    MV{1}.faceVertexCData = Ff(handles.matfile.geo.Mesh.vertices_front);
    
    % back
    MV{2}.faceVertexCData = Fb(handles.matfile.geo.Mesh.vertices_back);
else
    %% Ori
    % front
    MV{1}.faceVertexCData = valuesF;
    
    % back
    MV{2}.faceVertexCData = valuesB;
...
if ~isempty(hp)
    ...
    meshObject(1) = patch(MV{1},'parent',handles.tag_mesh);
    meshObject(2) = patch(MV{2},'parent',handles.tag_mesh);
    %%
    ...
    tt(1,1) = text(min(MV{1}.vertices(:,1)),max(MV{1}.vertices(:,2))+1,'RA');
    tt(2,1) = text(max(MV{1}.vertices(:,1)),max(MV{1}.vertices(:,2))+1,'LA','horizontalalignment','right');
    tt(3,1) = text(min(MV{1}.vertices(:,1))+range(MV{1}.vertices(:,1))/2,max(MV{1}.vertices(:,2))+1,'FRONT','fontweight','bold','horizontalalignment','center');
    
    tt(1,2) = text(min(MV{2}.vertices(:,1)),max(MV{1}.vertices(:,2))+1,'LA');
    tt(2,2) = text(max(MV{2}.vertices(:,1)),max(MV{1}.vertices(:,2))+1,'RA','horizontalalignment','right');
    tt(3,2) = text(min(MV{2}.vertices(:,1))+range(MV{2}.vertices(:,1))/2,max(MV{2}.vertices(:,2))+1,'BACK','fontweight','bold','horizontalalignment','center');
    
    set(tt(:),'verticalalignment','bottom')
    set(handles.tag_mesh,'xlim',[-3 60],'ylim',[-2 42],'box','off','position',[.02 .12 .45 .6])
...
if get(handles.tag_all_chan,'value')
    ...
    if isfield(handles.matfile,'signals_proc_Ensite_AT')
        ic2 = find(handles.matfile.signals_proc_Ensite_AT(samp,:)==pos(2));
    else
        ic2 = nan;
    end
    if isfield(handles.matfile,'signals_proc_Ensite_RT')
...
if get(handles.tag_modify_type,'value')==1
    ...
    i_po = find(sum(abs([po(1).Vertices(:,1) po(2).Vertices(:,1)]-handles.matfile.geo.xyz(ic(1),1))==0));
    if ic<=128
        po(i_po).FaceVertexCData = handles.matfile.Markers.dt(ib,1:128)'-handles.matfile.spikes(ib);
    else
        po(i_po).FaceVertexCData = handles.matfile.Markers.dt(ib,129:256)'-handles.matfile.spikes(ib);
    end
    ...
    for j = 1:size(ii,2)
        ...
        if ~isempty(tt)
            ...
            %         t = [1:size(handles.matfile.signals,1)]/handles.matfile.ParamSig.frequency*1000 - handles.matfile.spikes(1);
            [~,dtp] = min(abs(t-rt));
            rtp = round(handles.matfile.Markers.rt_Wyatt(ib,ic(j))/1000*handles.matfile.ParamSig.frequency);
    ...
    i_po = find(sum(abs([po(1).Vertices(:,1) po(2).Vertices(:,1)]-handles.matfile.geo.xyz(ic(1),1))==0));
    if ic<=128
        po(i_po).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,1:128)'-handles.matfile.spikes(ib);
    else
        po(i_po).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,129:256)'-handles.matfile.spikes(ib);
    end
    set(handles.tag_mesh,'clim',cb);
end
guidata(hObject,handles)
...
if isequal(get(info_struct.Target.Parent,'tag'),'tag_mesh')
    ...
    if handles.tag_menu.Value==1
        ...
        if im<=128
            po(2).FaceVertexCData = handles.matfile.Markers.dt(ib,1:128)'-handles.matfile.spikes(ib);
        else
            po(1).FaceVertexCData = handles.matfile.Markers.dt(ib,129:256)'-handles.matfile.spikes(ib);
        end
        
    elseif handles.tag_menu.Value==2
        handles.matfile.Markers.rt_Wyatt(ib,im) = nan;
        if im<=128
            po(2).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,1:128)'-handles.matfile.spikes(ib);
        else
            po(1).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,129:256)'-handles.matfile.spikes(ib);
        end
    elseif handles.tag_menu.Value==3
        handles.matfile.Markers.dt(ib,im) = nan;
        handles.matfile.Markers.rt_Wyatt(ib,im) = nan;
        if im<=128
            po(2).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,1:128)'-handles.matfile.Markers.dt(ib,1:128)';
        else
            po(1).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,129:256)'-handles.matfile.Markers.dt(ib,129:256)';
        end
    elseif handles.tag_menu.Value>3
        return
    end
...
elseif isequal(get(info_struct.Target.Parent,'type'),'axes')
    ...
    for i = 1:length(ii)
        if handles.tag_plot_filt.Value
            ...
            jj(i) = (handles.matfile.signals(round(handles.matfile.Markers.rt_Wyatt(ib,ii(i))/1000*handles.matfile.ParamSig.frequency),ii(i))==pos(2));
        end
    end
    im1 = ii((jj));
    handles.matfile.Markers.rt_Wyatt(ib,im1) = nan;
    %
    ...
    for i = 1:length(ii)
        ...
        jj(i) = (handles.matfile.signals(round(handles.matfile.Markers.dt(ib,ii(i))/1000*handles.matfile.ParamSig.frequency),ii(i))==pos(2));            
        end
    end
    im2 = ii((jj));
    handles.matfile.Markers.dt(ib,im2) = nan;
...
cb = get(handles.tag_mesh,'clim');
ic_tot = [im1 im2];
for i = 1:length(ic_tot)
    for i_po = 1:length(po)
        ic = ic_tot(i);
        if handles.tag_menu.Value==1
            if ic<=128
                po(i_po).FaceVertexCData = handles.matfile.Markers.dt(ib,1:128)'-handles.matfile.spikes(ib);
        ...
        elseif handles.tag_menu.Value==3
            if ic<=128
                po(i_po).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,1:128)'-handles.matfile.Markers.dt(ib);
            else
                po(i_po).FaceVertexCData = handles.matfile.Markers.rt_Wyatt(ib,129:256)'-handles.matfile.Markers.dt(ib);
            end
        end
    end
end
...
if ~(D==1|D==2|D==3)
   h = errordlg('D (number of neighbor to include into the RVI calculation) can only be equal to 1,2 or 3.'); 
   error('D (number of neighbor to include into the RVI calculation) can only be equal to 1,2 or 3.');
end
...
%     d = d(ii);
    ...
    d = sqrt(sum( (xyz  - repmat(xyz(ic,:),[size(xyz ,1),1])).^2 ,2));
    Dx = sqrt(sum( (xyz(:,1) - xyz(ic,1)).^2,2));
    Dy = sqrt(sum( (xyz(:,2) - xyz(ic,2)).^2,2));
  
    ii = find(Dx<=Dxm & Dy<=Dym);
    ii(Dx(ii)==0 & Dy(ii)==0)=[];
    d = d(ii);
    if ~isempty(ii)
...
% if handles.tag_view_mesh_interp.Value
%     Ff = scatteredInterpolant(handles.matfile.geo.Mesh.vertices_front(:,1),handles.matfile.geo.Mesh.vertices_front(:,2),values(:));
%     values_int  = F([po.Vertices(:,1),po.Vertices(:,2)]);
% else
    values_int  = values;
% end

[~,i_poF] = sort(mean([po(1).Vertices(:,1) po(2).Vertices(:,1)]),'ascend');

po(i_poF(1)).FaceVertexCData = values_int(1:128);
po(i_poF(2)).FaceVertexCData = values_int(129:256);

set(get(po(1),'parent'),'clim',[prctile(values_int,5) prctile(values_int,95)])
...
if isfield(handles.matfile,'signals_proc_Ensite_RT')
    handles_main.RVI = handles.matfile.signals_proc_Ensite_RT;
end
if isfield(handles.matfile,'signals_proc_Ensite_AT')
    handles_main.RVI = handles.matfile.signals_proc_Ensite_AT;
end
...
for i = 1:4
    ...
    title(cbar(i),colorLabel{i},'fontsize',10);
end
set(meshObject,'edgecolor','none','facecolor','interp','facelighting','gouraud',...
    'ambientstrength',0.9,'specularcolorreflectance',0.4,'specularexponent',3,'specularstrength',0.3);
light('Position',[0 0 1],'Style','infinite')
...
L = .2;H=.85;
set(ax(1),'position',[.05 .15 L H])
set(ax(2),'position',[.3 .15 L H])
set(ax(3),'position',[.55 .15 L H])
set(ax(4),'position',[.80 .15 L H])

set(cbar(1),'position',[.05 .10 L .025])
set(cbar(2),'position',[.3 .1 L .025])
set(cbar(3),'position',[.55 .1 L .025])
set(cbar(4),'position',[.80 .1 L .025])
...
aa = plot(handles.matfile.geo.xyz(ic,1),handles.matfile.geo.xyz(ic,2),'x');
