function varargout = GUI_EGM_map_viewer_CardioInsight_v3(varargin)
% GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3 MATLAB code for GUI_EGM_map_viewer_CardioInsight_v3.fig
%      GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3, by itself, creates a new GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3 or raises the existing
%      singleton*.
%
%      H = GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3 returns the handle to a new GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3 or the handle to
%      the existing singleton*.
%
%      GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3.M with the given input arguments.
%
%      GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3('Property','Value',...) creates a new GUI_EGM_MAP_VIEWER_CARDIOINSIGHT_V3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_EGM_map_viewer_CardioInsight_v3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_EGM_map_viewer_CardioInsight_v3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_EGM_map_viewer_CardioInsight_v3

% Last Modified by GUIDE v2.5 10-Sep-2021 11:22:51

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @GUI_EGM_map_viewer_CardioInsight_v3_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_EGM_map_viewer_CardioInsight_v3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_EGM_map_viewer_CardioInsight_v3 is made visible.
function GUI_EGM_map_viewer_CardioInsight_v3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_EGM_map_viewer_CardioInsight_v3 (see VARARGIN)

% Choose default command line output for GUI_EGM_map_viewer_CardioInsight_v3
handles.output = hObject;

%%
h = guidata(varargin{1});

if isfield(h,'signals_proc')
    handles.matfile.signals_proc = h.signals_proc;
else
    handles.matfile.signals_proc = h.signals;
end
...
if length(handles.matfile.ParamSig.Label)~=size(handles.matfile.signals,2)
    handles.matfile.ParamSig.Label = cell(1,size(handles.matfile.signals,2));
    for i =1:size(handles.matfile.signals,2)
        handles.matfile.ParamSig.Label{i}=['Node-',num2str(i)];
    end
end
...
% UIWAIT makes GUI_EGM_map_viewer_CardioInsight_v3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_EGM_map_viewer_CardioInsight_v3_OutputFcn(hObject, eventdata, handles)
...
if ~isempty(info_struct)&&isequal(info_struct(1).Target.Parent,handles.tag_mesh)
...
if length(pos)>1
    im = cellfun(@(x) find(sum([xyz - x]==0,2)),pos);
    handles.tag_plot_egm_numb_chan.String = '1';
else
    pos = pos{1};
    d = sqrt(sum( (xyz  - repmat(pos,[size(xyz ,1),1])).^2 ,2));
    [~,im] = sort(d,'ascend');
    im = im(1:str2double(get(handles.tag_plot_egm_numb_chan,'string')));
%     set(handles.tag_find_chan,'value',int16(im(1)));
    handles.tag_find_chan.Value = round(im(1));
...
if get(handles.tag_hold_on,'value')==1
    ...
    xx_lim = [handles.matfile.spikes(ib)-100 handles.matfile.spikes(ib)+500]-handles.matfile.spikes(1);
    cla(handles.tag_egm)
...
switch str{istr}
    ...
    case 'RVImin'
        ...
        if ~isfield(handles.matfile,'RVI')
            tag_Disp_Callback(hObject,[], handles);
            handles = guidata(hObject);
        end
        
        D = str2double(handles.tag_RVI_d.String);
        if handles.matfile.RVI.R~=D
            tag_Disp_Callback(hObject,[], handles);
            handles = guidata(hObject);
        end
        
        values = handles.matfile.RVI.RVImin(ib,:);
        colorLabel = ['RVI_{min} [R=',num2str(handles.matfile.RVI.R),'mm]'];
    ...
    case 'RVImean'
        ...
        values = handles.matfile.RVI.('RVImean')(ib,:);
        colorLabel = ['RVI_{avg} [R=',num2str(handles.matfile.RVI.R),'mm]'];
    ...
    case 'DispAT'
        ...
        D = str2double(handles.tag_RVI_d.String);
        if handles.matfile.Disp_Local.R~=D
            tag_Disp_Callback(hObject,[], handles);
            handles = guidata(hObject);
        end
        
        values = handles.matfile.Disp_Local.range_AT(ib,:);
        colorLabel = ['Disp(AT) (ms) [R=',num2str(handles.matfile.Disp_Local.R),'mm]'];
    ...
    case 'DispRT'
        ...
        colorLabel = ['Disp(RT) (ms) [R=',num2str(handles.matfile.Disp_Local.R),'mm]'];
    ...
    case 'DispARI'
        ...
        colorLabel = ['Disp(ARI) (ms) [R=',num2str(handles.matfile.Disp_Local.R),'mm]'];
    ...
    case 'GradAT'
        ...
        values = handles.matfile.Disp_Local.grad_mean_AT(ib,:);
        colorLabel = ['Grad(AT) (ms/mm) [R=',num2str(handles.matfile.Disp_Local.R),'mm]'];
    ...
    case 'GradRT'
        ...
        values = handles.matfile.Disp_Local.grad_mean_RT(ib,:);
        colorLabel = ['Grad(RT) (ms/mm) [R=',num2str(handles.matfile.Disp_Local.R),'mm]'];
    ...
    case 'GradARI'
        ...
        values = handles.matfile.Disp_Local.grad_mean_ARI(ib,:);
        colorLabel = ['Grad(ARI) (ms/mm) [R=',num2str(handles.matfile.Disp_Local.R),'mm]'];
end
...
dcm_obj.Enable = 'on';
dcm_obj.UpdateFcn = @myfunctioncursor_markers_EnsiteClassic_v2;


cla(handles.figure1)
axes(handles.tag_mesh);

% Sock (added 08/11/2019)
if isfield(handles.matfile.geo,'Sock_Info');
    v = nan(1,length(handles.matfile.geo.Sock_Info.channel_num));
    index_chan = [1:length(values)];
    for i= 1:length(values)
        a = find(index_chan(i) == handles.matfile.geo.Sock_Info.channel_num);
        if isnan(v(a))
            v(a) =values(i);
        end
    end
...
if ~isempty(hp)
    ...
    % other structures
    mesh_types = {'Aorta','Atria','PT','septum','LAD','Valves','LV_lead','RV_lead','Ao','RA','LA','Septum'};
    
    if isfield(handles.matfile.geo,'Nodes_anatomy')
        for i = 1:length(handles.matfile.geo.Name)
            handles.matfile.geo.(['Mesh_',handles.matfile.geo.Name{i}]) = handles.matfile.geo.Mesh_anatomy{i};
            handles.matfile.geo.(['Nodes_',handles.matfile.geo.Name{i}]) = handles.matfile.geo.Nodes_anatomy{i};
        end
    end
    ...
    for im = 1:length(mesh_types)
        if isfield(handles.matfile.geo,['Mesh_',mesh_types{im}])
            ...
            ind = ind+1;
            k = min(0.65+0.05*ind,.95);
            set(meshObject_other(im),'edgecolor','none','facecolor',[1 1 1]*k);
        else
    ...
    if exist([pwd,filesep,'GUI_egm_mFiles',filesep,'Geo_Chann',filesep,'colormap_carto.mat'],'file')>0
        load([pwd,filesep,'GUI_egm_mFiles',filesep,'Geo_Chann',filesep,'colormap_carto.mat'],'cmap')
        colormap(cmap)
    else
        cmap = jet;
        cmap = cmap(end:-1:1,:);
...
dcm_obj = datacursormode(handles.figure1);
info_struct = getCursorInfo(dcm_obj);
iiko = cellfun(@(x) get(x,'parent'),{info_struct.Target})==handles.tag_mesh;
info_struct(iiko) = [];
clear iiko
pos = info_struct.Position;
...
if ~isequal(cb,[-1 1])
    ...
    C =  load([pwd,filesep,'GUI_egm_mFiles',filesep,'Geo_Chann',filesep,'colormap_carto.mat']);
    cmap = C.cmap([1 2:2:end],:);
    colormap(handles.tag_mesh,cmap)
    set(handles.tag_mesh,'clim',[prctile(po.FaceVertexCData,5),prctile(po.FaceVertexCData,95)]);
    cbar = findobj(handles.figure1,'tag','Colorbar');
    set(cbar,'Limits',[prctile(po.FaceVertexCData,5),prctile(po.FaceVertexCData,95)],...
        'Ticks',10*round(prctile(po.FaceVertexCData,5)/10):20:10*round(prctile(po.FaceVertexCData,95)/10),...
        'TickLabels',10*round(prctile(po.FaceVertexCData,5)/10):20:10*round(prctile(po.FaceVertexCData,95)/10))
    clear cbar C
end
guidata(hObject,handles)
...
elseif isequal(get(info_struct.Target.Parent,'type'),'axes')
    ...
    for i = 1:length(ii)
        if handles.tag_plot_filt_RT.Value
            ...
            jj(i) = (handles.matfile.signals_proc_AT(round(handles.matfile.Markers.rt_Wyatt(ib,ii(i))/1000*handles.matfile.ParamSig.frequency),ii(i))==pos(2));
    ...
    for i = 1:length(ii)
        if handles.tag_plot_filt_RT.Value
            jj(i) = (handles.matfile.signals_proc(round(handles.matfile.Markers.dt(ib,ii(i))/1000*handles.matfile.ParamSig.frequency),ii(i))==pos(2));
    ...
    handles.matfile.Markers.dt(ib,ii((jj))) = nan;
...
% --- Executes on button press in tag_Disp.
function tag_Disp_Callback(hObject, eventdata, handles)
% hObject    handle to tag_Disp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
[RVI,Disp_Local] = RVI_calculation_fun(dt,rt,xyz,D);
handles.matfile.Disp_Local = Disp_Local.(['D_',num2str(D)]);
handles.matfile.RVI = RVI.(['D_',num2str(D)]);
handles.matfile.RVI.R = D;
handles.matfile.Disp_Local.R = D;
...
% --- Executes on button press in tag_outliers.
function tag_outliers_Callback(hObject, eventdata, handles)
% hObject    handle to tag_outliers (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

if isequal(get(handles.tag_mesh,'view'),[0 90])
set(handles.tag_mesh,'view',[1 91]);
end
    
po = findobj(handles.figure1,'type','patch');
po = findobj(po,'facecolor','interp');
x = get(po,'FaceVertexCData');

xo = zeros(size(x));
w = 1;
xo(x<prctile(x,25)-w*iqr(x))=-1;
xo(x>prctile(x,75)+w*iqr(x))=1;

switch handles.tag_menu.String{handles.tag_menu.Value}
    case 'Activ. T'
        xo(x>180) = 1;
    case 'Repol. T'
        xo(x>500) = 1;
...
% cmap = colormap(handles.tag_mesh);
cmap2 = [1 0 0;0 1 0;1 0 1];
po = findobj(handles.figure1,'type','patch');
po = findobj(po,'facecolor','interp');
set(po,'FaceVertexCData',xo(:))
set(po.Parent,'clim',[-1 1])
cbar = findobj(handles.figure1,'tag','Colorbar');
cbar.Limits = [-1 1];
colormap(cmap2)
cbar.Ticks = [-1 0 1];
cbar.TickLabels = {'Low','Normal','High'};
handles.tag_mesh_atria.Value = 0 ;
tag_mesh_atria_Callback(hObject,[],handles);
handles.tag_nodes.Value = 1;
tag_nodes_Callback(hObject,[],handles);


figure
ax(1) = subplot(211);
plot(x(xo==0),0.75+rand(1,sum(xo==0))/10,'ok','markerfacecolor','g');
hold on
plot(x(xo==-1),0.75+rand(1,sum(xo==-1))/10,'ok','markerfacecolor','r');
plot(x(xo==1),0.75+rand(1,sum(xo==1))/10,'ok','markerfacecolor',[1 0 1]);

hold on
boxplot(x,'Orientation','horizontal','Whisker',w);

ax(2) = subplot(212);
plot([1:size(handles.matfile.signals,1)]/handles.matfile.ParamSig.frequency*1000-handles.matfile.spikes,handles.matfile.signals,'k');
xlabel('Time (ms)')

set(ax,'xlim',[-50 600])
linkaxes(ax,'x');
set(ax,'xtick',[-100:100:1000],'xgrid','on')
set(ax(1),'ytick',[])
set(gcf,'units','centimeters','paperunits','centimeters','position',[3 5 15 10],'paperPosition',[0 0 15 10],'papersize',[15 10]);
set(ax(2),'position',[.10 .10 .85 .50])
set(ax(1),'position',[.10 .70 .85 .25],'ylim',[0.70 1.15])


%
answer = questdlg('Do you want to plot ...', ...
    'Outliers','Extremely Small values', ...
    'Extremely Large values','Nothing','Nothing');

switch answer
    case 'Extremely Large values'
        ii = find(xo==1);
        if ~isempty(ii)
            xyz0 = handles.matfile.geo.Nodes_ventricles(ii,:);
            hb = waitbar(0,'... tagging points');
            for j = 1:length(ii)
                waitbar(j/length(ii),hb);
                datatip(po,xyz0(j,1),xyz0(j,2),xyz0(j,3));
            end
            close(hb)
            tag_plot_egm_Callback(hObject,[],handles);
            clear i
        end
    case 'Extremely Small values'
        ii = find(xo==-1);
        if ~isempty(ii)
            xyz0 = handles.matfile.geo.Nodes_ventricles(ii,:);
            hb = waitbar(0,'... tagging points');
